...
- **Discord bot** — `DISCORD_TOKEN`, guild-only `/buddytrails-verify` (code flow), hourly 3★ DM + daily 09:00 Asia/Bangkok push (`brief.daily` + schedule) per linked user (`DAILY_PUSH_HOUR` env to override) + dynamic automations (`automation.create/list/update/delete` with `name`, `message`, `rrule`, `dtstart` — e.g. `FREQ=DAILY` or `FREQ=WEEKLY;BYDAY=MO`)
...
| `automation.*` | `create`, `list`, `update`, `delete` (dynamic RRULE + custom message → Discord DM) |
| `brief.*` | `daily`, `weekly` |
| `search.*` | `all` |
