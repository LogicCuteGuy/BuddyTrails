...
- `/buddytrails-link <openwebui_user>` — **works in DMs and guilds, no guild needed**. Links your Open WebUI `X-User-Id` (e.g. email) to your Discord account. Hourly scheduler DMs each linked user their own 3★ due-soon tasks. Without a link, DM-setup users still get DMs (fallback to Discord ID).
...
DB file `buddytrails.db` is created on first run. Single file, no cloud sync. Existing DBs are migrated automatically (adds `user_id`, `created_by`, `user_discord_link`, fixes `conversation_settings` PK).
...
- **Discord DMs not arriving** — run `/buddytrails-link <your Open WebUI user>` first, then create a 3★ task with deadline today.
