...
export const tools: ToolDef[] = [
  ...
  {
    name: "notify.send",
    description: "Send a Discord notification (AI-initiated). Target 'dm' sends to linked user's DM (requires link.create + /buddytrails-verify), 'channel' sends to a Discord channel ID. Use for AI chat bot messages, reminders, or alerts via Discord.",
    inputSchema: z.object({
      message: z.string().min(1).max(2000),
      target: z.enum(["dm", "channel"]).default("dm"),
      channelId: z.string().optional(),
      discordUserId: z.string().optional(),
    }),
    handler: async ({ message, target = "dm", channelId, discordUserId }) => {
      const { sendDiscordMessage } = await import("./discord/notify.js");
      return sendDiscordMessage({ target, channelId, discordUserId, message });
    },
  },
  {
    name: "health",
